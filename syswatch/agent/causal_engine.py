 ...
 def evaluate(self):
  ...
  for name,req in RULES.items():
   if set(req)<=present:
    ...
    active[name]={'name':name.replace('_',' ').title(),'stage':STAGE[name],'confidence':round(conf,3),'tactic':'MITRE ATT&CK mapped behavioral chain','signals':matched,'last_seen':max(e['ts'] for e in matched)}
  self.chains=active
 def summary(self):
  self.evaluate(); active=list(self.chains.values()); conf=max([c['confidence'] for c in active],default=0); score=min(100,int(conf*100)); level='CRITICAL' if score>=85 else 'HIGH' if score>=65 else 'MEDIUM' if score>=30 else 'LOW'; return {'score':score,'confidence':conf,'level':level,'chains':active,'events':list(self.events)[-100:]}
engine=Engine()
